...
class PSWebsocketClient:
    ...
    async def challenge_user(self, user_to_challenge: str, battle_format: Format):
        logger.info("Challenging {}...".format(user_to_challenge))
        message = ["/challenge {},{}".format(user_to_challenge, battle_format.value)]
        await self.send_message("", message)
        self.last_challenge_time = time.time()

    async def accept_challenge(self, battle_format: Format, room_name: str | None):
        if room_name is not None:
            await self.join_room(room_name)

        logger.info("Waiting for a {} challenge".format(battle_format.value))
        username = None
        while username is None:
            ...
            if (
                ...
                and split_msg[5] == battle_format.value
            ):
